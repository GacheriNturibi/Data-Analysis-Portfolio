...
# install.packages("ggplot2") # For nice visualizations
# install.packages("viridis") # For color scales in plots
# install.packages("dplyr")   # For data manipulation (e.g., mutate)
#############################################################################################################################################################################
...
#############################################################################################################################################################################
# --- PART 12: Perform Ordinary Kriging for Direct Comparison ---
# This section performs Ordinary Kriging on the original 'zinc' variable
# to allow for a direct comparison with Regression Kriging.
#############################################################################################################################################################################

cat("--- PART 12: Performing Ordinary Kriging for Direct Comparison ---\n\n")

# Calculate the empirical variogram for original zinc data
vgm_ok_empirical <- variogram(zinc ~ 1, data = meuse,
                              cutoff = 1500, width = 100)

cat("Step 12a: Empirical Variogram for OK Calculated (First 6 Rows):\n")
print(head(vgm_ok_empirical))
cat("\n")

# Fit a theoretical variogram model for original zinc data
vgm_ok_model <- fit.variogram(vgm_ok_empirical, model = vgm("Sph", psill = 100000, range = 800, nugget = 10000))

cat("Step 12b: Fitted Theoretical Variogram Model Parameters for OK:\n")
print(vgm_ok_model)
cat("\n")
...
cat("Step 12c: Ordinary Kriging Performed (First 6 OK Predicted Points):\n")
print(head(as.data.frame(meuse.grid), 6))
cat("\n")

# Visualize the Ordinary Kriging Prediction Map
...
print(p_ok_pred)
cat("Step 12d: Displayed Ordinary Kriging Prediction Map.\n\n")

#############################################################################################################################################################################
# --- PART 13: Compare OK and RK Accuracy using Cross-Validation ---
# Cross-validation assesses the predictive performance of the models.
#############################################################################################################################################################################

cat("--- PART 13: Comparing OK and RK Accuracy using Cross-Validation ---\n\n")

# Perform K-fold cross-validation for Ordinary Kriging
# 'nfold' can be adjusted, e.g., to the number of samples for Leave-One-Out CV
ok_cv <- krige.cv(zinc ~ 1, locations = meuse, model = vgm_ok_model, nfold = 5)

# Perform K-fold cross-validation for Regression Kriging
rk_cv <- krige.cv(zinc ~ dist + ffreq, locations = meuse, model = vgm_residuals_model, nfold = 5)

# Calculate performance metrics
# Mean Error (ME): Should be close to 0 for unbiased predictions
# Root Mean Squared Error (RMSE): Overall accuracy, lower is better
# Mean Standardized Squared Error (MSSE): Should be close to 1 for correct variance estimation

# For Ordinary Kriging
ok_me <- mean(ok_cv$residual)
ok_rmse <- sqrt(mean(ok_cv$residual^2))
ok_msse <- mean(ok_cv$zscore^2)

# For Regression Kriging
rk_me <- mean(rk_cv$residual)
rk_rmse <- sqrt(mean(rk_cv$residual^2))
rk_msse <- mean(rk_cv$zscore^2)

# Create a comparison table
comparison_table <- data.frame(
  Method = c("Ordinary Kriging (OK)", "Regression Kriging (RK)"),
  Mean_Error = c(ok_me, rk_me),
  RMSE = c(ok_rmse, rk_rmse),
  MSSE = c(ok_msse, rk_msse)
)

cat("Step 13a: Cross-Validation Performance Comparison:\n")
print(comparison_table)
cat("\n")

cat("Step 13b: Interpretation of Cross-Validation Results:\n")
cat("- Mean Error (ME): Values closer to 0 indicate less bias in predictions.\n")
cat("- RMSE: Lower values indicate higher accuracy.\n")
cat("- MSSE: Values closer to 1 suggest that the prediction variances are well-estimated.\n")
cat("  If MSSE < 1, the model might be overestimating uncertainty.\n")
cat("  If MSSE > 1, the model might be underestimating uncertainty.\n\n")


#############################################################################################################################################################################
# --- PART 14: Brief Interpretation of Results ---
#############################################################################################################################################################################

cat("--- Interpretation --- \n")
cat("1. **Ordinary Kriging (OK)** interpolates based solely on the spatial correlation of the variable itself.\n")
cat("2. **Regression Kriging (RK)** first models a large-scale deterministic trend using auxiliary variables (e.g., distance to river, flood frequency) and then kriges the *residuals* (the unexplained variation).\n")
cat("3. The Deterministic Trend Map (Step 5c) shows how zinc concentration changes predictably with environmental factors.\n")
cat("4. The Residuals Map (Step 6b) shows the spatial pattern of the variation not explained by the linear model.\n")
cat("5. The Variogram of Residuals (Step 8c) models the spatial correlation of this unexplained variation, which is often 'purer' than the original variogram.\n")
cat("6. The final Regression Kriging Prediction Map (Step 11a) is the sum of the deterministic trend and the kriged residuals.\n")
cat("7. The Prediction Variance Maps (Step 11b for RK, Step 12d for OK) indicate the uncertainty. Areas far from samples generally have higher variance.\n")
cat("8. **Comparison (Part 13):** By examining the RMSE from cross-validation, you can see which method (OK or RK) provides more accurate predictions for this dataset. If RK has a lower RMSE, it suggests that incorporating the environmental covariates significantly improved the predictive power. The MSSE indicates how well the models estimate their own uncertainty.\n")
cat("\nThis exercise demonstrates the steps of Regression Kriging and provides a method to compare its accuracy against Ordinary Kriging, highlighting the benefits of integrating environmental covariates when strong trends exist.\n")